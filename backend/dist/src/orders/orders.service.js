...
let OrdersService = OrdersService_1 = class OrdersService {
    ...
    async create(userId, createOrderDto) {
        ...
        if (address.userId !== userId) {
            throw new common_1.ForbiddenException('Address does not belong to user');
        }
        ...
        for (const item of cart.items) {
            ...
            if (!product.isActive) {
                throw new common_1.BadRequestException(`Product ${product.name} is no longer available`);
            }
            const availableQuantity = product.inventory?.quantity || 0;
            if (availableQuantity < item.quantity) {
                throw new common_1.BadRequestException(`Insufficient stock for ${product.name}. Available: ${availableQuantity}`);
            }
